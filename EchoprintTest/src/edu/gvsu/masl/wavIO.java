...
public class wavIO {
	...
	public long myChunkSize;
	public long mySubChunk1Size;
	public int myFormat;
	public long myChannels;
	public long mySampleRate;
	public long myByteRate;
	public int myBlockAlign;
	public int myBitsPerSample;
	public long myDataSize;
	...
	public boolean read() {
		...
		try {
			...
			System.out.println("data size:" + myDataSize);
			// read the data chunk
	...
	public boolean save(FileOutputStream outFile) {
		try {
...
//			 samples per second (numbers per second)
	...
	public void turntoint() {
		intdata = new int[myData.length / 2];
		byte[] bar = new byte[2];
		for (int i = 0; i < intdata.length; i++) {
			bar[0] = myData[2 * i];
			bar[1] = myData[2 * i + 1];
			intdata[i] = wavIO.byteArrayToInt(bar);
		}
	}
	...
	public void normalize() {
		...
		int width = (max - min) / 2;
		for (int i = 0; i < intdata.length; i++) {
			doubledata[i] = (intdata[i] - width) / width;
		}
	}
}